...
##add page
##Since other code didnt work use this to link to non functional website add page
@app.route('/add')
def add():
...
class Project(db.Model, UserMixin):
    ...
    level = db.Column(db.String(2000), nullable=False)
    course = db.Column(db.String(2000), nullable=False)
...
#############################################
##Code that doesnt work for adding projects##
#############################################
...
##def validate_project(self, name):
        ##projname = Project.query.filter_by(name=name.data).first()
        ##if projname:
            ##raise ValidationError('A project with that name has already been made please name it something else.')
